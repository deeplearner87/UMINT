...
rna <- read.csv("MALT10k_rna_scaled.csv", header=TRUE, row.names=1)
protein <- read.csv("MALT10k_adt_scaled.csv", header=TRUE, row.names=1)
rna <- data.matrix(rna)
...
outfile = file.path(getwd(),"MALT10k_model.hdf5")
#model <- run_mofa(MOFAobject, outfile)
MALT10k_model <- run_mofa(MOFAobject, outfile, use_basilisk=TRUE)
