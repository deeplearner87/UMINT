...
rna <- read.csv("bmcite30k_rna_scaled.csv", header=TRUE, row.names=1)
protein <- read.csv("bmcite30k_adt_scaled.csv", header=TRUE, row.names=1)
rna <- data.matrix(rna)
...
outfile = file.path(getwd(),"bmcite30k_model.hdf5")
#model <- run_mofa(MOFAobject, outfile)
bmcite30k_model <- run_mofa(MOFAobject, outfile, use_basilisk=TRUE)
